...
function DescriptionImage({ className, src, ...props }: React.ComponentProps<"img">) {
  ...
  const { cleanSrc, width, height } = useMemo(() => {
    if (!src) return { cleanSrc: src };
    const hashIdx = src.indexOf("#dim=");
    if (hashIdx === -1) return { cleanSrc: src };
  ...
  return (
    <>
      {!loaded && (
        <span
          className="block animate-pulse rounded-lg bg-muted"
          style={
            width && height
