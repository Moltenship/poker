import { convexTest } from "convex-test";

import { api } from "../_generated/api";
import schema from "../schema";

const modules = import.meta.glob("../**/*.ts");

async function createTestRoom(t: ReturnType<typeof convexTest>) {
  return await t.run(
    async (ctx) =>
      await ctx.db.insert("rooms", {
        name: "Test Room",
        roomCode: "TESTCODE",
        cardSet: ["1", "2", "3", "5", "8"],
        status: "lobby",
        currentTaskIndex: 0,
        createdBy: "session-1",
        createdAt: Date.now(),
      }),
  );
}

describe("tasks", () => {
  it("addTask creates a manual task with auto-incremented order", async () => {
    const t = convexTest(schema, modules);
    const roomId = await createTestRoom(t);

    const taskId1 = await t.mutation(api.tasks.addTask, {
      roomId,
      sessionId: "session-1",
      title: "User Story 1",
    });
    const taskId2 = await t.mutation(api.tasks.addTask, {
      roomId,
      sessionId: "session-1",
      title: "User Story 2",
    });

    const tasks = await t.query(api.tasks.getTasksForRoom, { roomId });
    expect(tasks).toHaveLength(2);
    expect(tasks[0].order).toBeLessThan(tasks[1].order);
    expect(tasks[0].isManual).toBeTruthy();
    expect(taskId1).toBeDefined();
    expect(taskId2).toBeDefined();
  });

  it("getTasksForRoom returns tasks ordered by order field", async () => {
    const t = convexTest(schema, modules);
    const roomId = await createTestRoom(t);

    await t.mutation(api.tasks.addTask, { roomId, sessionId: "s1", title: "Task A" });
    await t.mutation(api.tasks.addTask, { roomId, sessionId: "s1", title: "Task B" });
    await t.mutation(api.tasks.addTask, { roomId, sessionId: "s1", title: "Task C" });

    const tasks = await t.query(api.tasks.getTasksForRoom, { roomId });
    const orders = tasks.map((t) => t.order);
    expect(orders).toStrictEqual([...orders].sort((a, b) => a - b));
  });

  it("setHoursEstimate persists hours", async () => {
    const t = convexTest(schema, modules);
    const roomId = await createTestRoom(t);
    const taskId = await t.mutation(api.tasks.addTask, {
      roomId,
      sessionId: "s1",
      title: "Task",
    });

    await t.mutation(api.tasks.setHoursEstimate, {
      hours: 4,
      sessionId: "s1",
      taskId,
    });

    const tasks = await t.query(api.tasks.getTasksForRoom, { roomId });
    expect(tasks[0].hoursEstimate).toBe(4);
  });

  it("setCurrentTask updates room currentTaskIndex", async () => {
    const t = convexTest(schema, modules);
    const roomId = await createTestRoom(t);

    await t.mutation(api.tasks.addTask, { roomId, sessionId: "s1", title: "T1" });
    await t.mutation(api.tasks.addTask, { roomId, sessionId: "s1", title: "T2" });
    await t.mutation(api.tasks.addTask, { roomId, sessionId: "s1", title: "T3" });

    await t.mutation(api.tasks.setCurrentTask, {
      roomId,
      sessionId: "s1",
      taskIndex: 2,
    });

    const room = await t.run(async (ctx) => ctx.db.get(roomId));
    expect(room?.currentTaskIndex).toBe(2);
  });

  it("deleteTask removes manual tasks", async () => {
    const t = convexTest(schema, modules);
    const roomId = await createTestRoom(t);

    const taskId = await t.mutation(api.tasks.addTask, {
      roomId,
      sessionId: "s1",
      title: "Manual Task",
    });
    await t.mutation(api.tasks.deleteTask, { sessionId: "s1", taskId });

    const tasks = await t.query(api.tasks.getTasksForRoom, { roomId });
    expect(tasks).toHaveLength(0);
  });

  it("deleteTask throws for imported (non-manual) tasks", async () => {
    const t = convexTest(schema, modules);
    const roomId = await createTestRoom(t);

    // Insert a non-manual task directly
    const taskId = await t.run(
      async (ctx) =>
        await ctx.db.insert("tasks", {
          roomId,
          jiraKey: "PROJ-1",
          title: "Jira Story",
          order: 1,
          isManual: false,
        }),
    );

    await expect(t.mutation(api.tasks.deleteTask, { sessionId: "s1", taskId })).rejects.toThrow();
  });
});
